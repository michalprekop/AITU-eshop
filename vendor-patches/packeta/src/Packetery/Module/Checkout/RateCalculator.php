...
class RateCalculator {
	...
	public function getShippingRateCost(
		...
		if ( ! empty( $carrierOptions['free_shipping_limit'] ) && is_numeric( $carrierOptions['free_shipping_limit'] ) ) {
			$freeShippingLimit = $this->currencySwitcherService->getConvertedPrice( (float) $carrierOptions['free_shipping_limit'] );
			$this->diagnosticsLogger->log(
