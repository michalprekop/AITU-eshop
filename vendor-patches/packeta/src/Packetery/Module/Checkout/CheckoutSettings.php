...
class CheckoutSettings {
	...
	private function getCarDeliveryExpeditionDay(): ?string {
		...
		if ( $carrierId === null || $this->carDeliveryConfig->isCarDeliveryCarrier( $carrierId ) === false ) {
			return null;
		}

		$optionId       = $this->checkoutService->getShippingMethodOptionId( $chosenShippingMethod );
		$carrierOptions = $this->carrierOptionsFactory->createByOptionId( $optionId )->toArray();
		$today          = new DateTime();
		$processingDays = isset( $carrierOptions['days_until_shipping'] ) && is_numeric( $carrierOptions['days_until_shipping'] )
			? (int) $carrierOptions['days_until_shipping']
			: 0;
		$cutoffTime     = isset( $carrierOptions['shipping_time_cut_off'] ) && is_string( $carrierOptions['shipping_time_cut_off'] ) && '' !== $carrierOptions['shipping_time_cut_off']
			? $carrierOptions['shipping_time_cut_off']
			: null;
