...
/**
 * Determine whether current frontend language context is Slovak.
 *
 * @return bool
 */
...
/**
 * Shared Packeta home delivery label in Slovak.
 *
 * @return string
 */
function aitu_packeta_home_delivery_title_sk() {
	return 'Doručenie kuriérom na adresu';
}

/**
 * Shared Packeta home delivery label in English.
 *
 * @return string
 */
function aitu_packeta_home_delivery_title_en() {
	return 'Delivery by courier to your address';
}

/**
 * Translate generic Packeta home delivery labels by current language.
 *
 * @param array<string, WC_Shipping_Rate> $rates Package rates.
 * @return array<string, WC_Shipping_Rate>
 */
function aitu_translate_packeta_home_delivery_rates( $rates ) {
	if ( ! is_array( $rates ) || empty( $rates ) ) {
		return $rates;
	}

	$label_sk = aitu_packeta_home_delivery_title_sk();
	$label_en = aitu_packeta_home_delivery_title_en();
	$target   = aitu_is_slovak_context() ? $label_sk : $label_en;
	...
	foreach ( $rates as $rate_key => $rate ) {
		...
		if ( $label_sk !== $label && $label_en !== $label ) {
			continue;
		}

		if ( method_exists( $rate, 'set_label' ) ) {
			$rate->set_label( $target );
		} else {
			$rate->label = $target;
		}
...
add_filter( 'woocommerce_package_rates', 'aitu_translate_packeta_home_delivery_rates', 200, 1 );
